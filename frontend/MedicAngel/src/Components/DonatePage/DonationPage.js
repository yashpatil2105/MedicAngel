...
const DonationPage = () => {
    ...
    const [popUpDonars, setPopUpDonars] = useState(false)
        ...
        <div className={`container emp-profile ${duringPopUp}`} >
            <div className="row">
                <div className="col-md-4">
                    ...
                    <b>Donar</b>
                    <br></br>
                    <button className='btn btn-primary' onClick={() => setPopUpDonars(true)}>Click to View Donars</button>
                        {popUpDonars && <DonarsList setPopUpDonars={setPopUpDonars} noOfDonar={campaign.noOfDonar} id={id} />}
                    {/* <select name="donate" size="auto" style={{ width: "200px" }}>
                        {donars.map((donar) => (
                            <option value="Merceders"> <tr>
                                <td style={{ textAlign: 'left' }}>{donar.name}</td>
                                <td style={{ textAlign: 'right' }}>Rs. {donar.amount}</td>
                            </tr> </option>
                        ))}
                    </select> */}
